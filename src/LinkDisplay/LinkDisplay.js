...
function LinkDisplay(props) {
    ...
    return (
        <div className={props.from=="page"?"link-display-page":"link-display-panel"}>
            ...
            <div className={props.from=="page"?"link-display-page-content":""}>
                <div className="link-display-id-holder">
                    <LinkDisplayId username={props.from=="page"?username:props.username}/>
                </div>
                <div>
                    {props.from==="display" && props.links && props.links.map((eachLink) => {
                        if (eachLink.enable && eachLink.name && eachLink.url) {
                            return <LinkDisplaySite linkData={eachLink}/>
                        }
                       })
                    }
                    {props.from==="page" && localLinks && localLinks.map((eachLink) => {
                        if (eachLink.enable && eachLink.name && eachLink.url) {
                            return <LinkDisplaySite linkData={eachLink}/>
                        }
                       })
