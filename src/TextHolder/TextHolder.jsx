...
function TextHolder(props) {
  function handleOnChange(e) {
    if (
      props.field === 'login-username' ||
      props.field === 'register-username'
    ) {
      ...
      props.field === 'login-password' ||
      props.field === 'register-password'
    ) {
      props.modifyUserDetails({
  ...
  return (
    <div className="text-holder">
      <input
        ...
        type={props.field.includes('password') ? 'password' : 'text'}
      />
    </div>
  );
}

const mapStatetoProps = (state) => ({ ...state.userDetailsReducer });
