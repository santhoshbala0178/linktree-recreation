...
export const userDetailsReducer = (state={username:"", password:"", repeatPassword:"", login:false, id:""}, actions) => {
  switch(actions.type) {
    ...
    case "RESET_DETAILS":
      return {...state, username: "", password:"", repeatPassword:"", login:false}
    default:
      return state
  }
}
...
export const newLinkReducer = (state= {links: []}, actions) => {
  switch (actions.type) {
    ...
    case "RESET_ALL_LINK":
      return {...state, link: []}
    case "EDIT_LINK_NAME":
      return {...state, links: state.links.map((eachLink) => {
        if (eachLink.id === actions.id) {
          eachLink.name = actions.name
          return eachLink
        }
        return eachLink
      })}
    ...
    case "ENABLE_LINK":
      return {...state, links: state.links.map((eachLink) => {
        if (eachLink.id === actions.id) {
          eachLink.enable = actions.enable
          return eachLink
        }
        return eachLink
      })}
    case "DELETE_LINK":
      return {...state, links: state.links.filter((eachLink) => {
       return (eachLink.id !== actions.id) 
      })}
    default: 
      return state;
  }
